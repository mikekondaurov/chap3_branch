...
export module vector; // defining the module called "Vector"
                      // использование модулей определено в с20
import <stdexcept>;
...
///	:elem{ new double[s] }, sz{ s } // initialize members
{
  // инвариант конструктора определяет какие параметры правильные для класса
	if (s < 0)
	  throw length_error{ "Vector constructor: отрицательный размер" };
	elem = new T[s];
	sz = s;
...
export  template<typename T> bool CompareVec(Kmk_Vector<T>& v1, Kmk_Vector<T>& v2)
